...
export const Navbar: React.FC<NavbarProps> = ({ currentRole, onGoHome, isAtHome, username }) => {
  return (
    <nav className="sticky top-0 z-50 bg-slate-900/95 backdrop-blur-md border-b border-white/5 px-4 md:px-6 py-2 md:py-3 flex items-center justify-between shadow-xl">
      <div className="flex items-center gap-3 md:gap-6">
        <button 
          onClick={onGoHome}
          className="flex items-center gap-2 md:gap-3 hover:opacity-80 transition-opacity group"
        >
          <div className="w-8 h-8 md:w-10 md:h-10 bg-white rounded-lg md:rounded-xl flex items-center justify-center shadow-lg shadow-white/5 transition-transform group-hover:scale-105">
            <svg viewBox="0 0 512 512" className="w-5 h-5 md:w-7 md:h-7 text-slate-950 drop-shadow-sm transition-transform group-hover:rotate-45 duration-700" fill="currentColor">
          ...
          <h1 className="text-lg md:text-xl font-black tracking-tighter uppercase italic">J<span className="text-indigo-500">S</span></h1>
        </button>

        {!isAtHome && (
          <button 
            onClick={onGoHome}
            className="flex items-center gap-2 bg-slate-800 hover:bg-slate-700 px-3 md:px-4 py-1.5 rounded-lg md:rounded-xl border border-white/5 text-[9px] md:text-xs font-black transition-all uppercase tracking-widest"
          >
      ...
      <div className="flex items-center gap-3">
        <div className="flex flex-col items-end hidden sm:flex">
          <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest leading-none">USUARIO</p>
          <p className="text-[10px] font-bold text-white">{username?.split(' ')[0] || 'ADMIN'}</p>
        </div>
        <div className={`${ROLE_CONFIG[currentRole].color} text-white px-3 md:px-4 py-1.5 md:py-2 rounded-lg md:rounded-xl text-[8px] md:text-[10px] font-black shadow-lg uppercase tracking-widest border border-white/10`}>
          {ROLE_CONFIG[currentRole].label.split(' ')[0]}
        </div>
      </div>
    </nav>
  );
