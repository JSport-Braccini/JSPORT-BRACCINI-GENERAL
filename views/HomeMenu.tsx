...
export const HomeMenu: React.FC<HomeMenuProps> = ({ role, onSelectModule }) => {
  const getMenuOptions = (): MenuOption[] => {
    ...
    if (role === UserRole.ADMIN || role === UserRole.REFEREE) {
      options.push({
        id: 'ADMIN_CONSOLE',
    ...
    // Estadísticas disponible para todos los roles
    options.push({
      id: 'GLOBAL_STATS',
      title: 'Estadísticas',
      description: 'Líderes y rendimiento global',
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto min-h-[calc(100vh-80px)] flex flex-col justify-center">
      <div className="mb-10 text-center">
        <h2 className="text-3xl md:text-5xl font-black uppercase italic tracking-tighter mb-2 animate-pop">
          Bienvenido, <span className="text-indigo-500">{role}</span>
        </h2>
        <p className="text-slate-500 text-[10px] font-black uppercase tracking-[0.4em]">SISTEMA CENTRAL JSPORT MASTER</p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5 animate-pop">
        {getMenuOptions().map((opt) => (
          <button
            key={opt.id}
            onClick={() => onSelectModule(opt.id)}
            className="group relative overflow-hidden bg-slate-900/50 border border-white/5 rounded-[2rem] p-6 text-left transition-all hover:border-indigo-500/50 hover:bg-slate-900 shadow-2xl"
          >
            <div className={`w-12 h-12 ${opt.color} rounded-2xl flex items-center justify-center mb-5 shadow-lg group-hover:scale-110 transition-transform`}>
              {opt.icon}
            </div>
            <h3 className="text-xl font-black uppercase tracking-tight mb-1 group-hover:text-indigo-400 transition-colors leading-none italic">
              {opt.title}
            </h3>
            <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest">{opt.description}</p>
          </button>
        ))}
      </div>
    </div>
